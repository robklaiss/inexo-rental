(function () {
    ...
    return `
      <div class="app-cart-order-footer">
        ...
        <div class="app-cart-order-actions">
          <a href="${escapeHtml(appUrl('/productos'))}" class="button w-button">seguir viendo</a>
  ...
  function updateCheckoutTotals() {
    ...
    if (freightNode) {
      freightNode.textContent = freight !== null && quote?.truckSelected ? formatMoney(freight) : 'por confirmar';
    }
    if (routeNode) {
      routeNode.textContent = freight !== null && quote?.truckSelected
        ? `Ruta Inexo a entrega: ${quote.distance.toFixed(2)} km ida · ${quote.roundTripDistance.toFixed(2)} km ida y vuelta · ${formatMoney(quote.perKm)}/km`
        : 'En el próximo paso del checkout podrás seleccionar direccion y tipo de camión para calcular el envío.';
    }
    if (totalNode) {
  ...
  function setupConditionalZone(form) {
    const citySelect = form.querySelector('[data-delivery-city]');
  ...
  document.querySelectorAll('.app-register-form').forEach((form) => {
    setupConditionalZone(form);
  });
