...
function app_setting_defaults(): array
{
    return [
        ...
        'tax_name' => 'ITBIS',
        'tax_rate' => '16',
        'hero_image' => ASSET_BASE . '/images/imagen-banner-central.avif',
...
function seed_blog_posts(): void
{
    $posts = [
        [
            'inexo-rental-alquiler-de-equipos',
            'Inexo Rental: equipos listos para cada etapa de tu obra',
            'Conoce como Inexo Rental facilita el acceso a equipos confiables para construccion.',
            ASSET_BASE . '/images/trabajadores-de-obra.avif',
            "Contar con el equipo correcto en el momento correcto evita atrasos y costos innecesarios.\n\nInexo Rental acompana proyectos de construccion con una seleccion de equipos para movimiento de suelo, compactacion, corte, energia, acceso y altura.\n\nNuestro proceso permite solicitar alquileres, organizar la entrega o el retiro y recibir una proforma clara antes de confirmar.",
            'Alquiler de equipos para construccion con Inexo Rental.',
        ],
        [
            'ventajas-alquiler-equipos-construccion',
            'Ventajas del alquiler de equipos para construccion',
            'Por que alquilar puede mejorar el flujo de caja, la productividad y la planificacion.',
            ASSET_BASE . '/images/especializaciones-movimiento-de-suelo.jpg',
            "El alquiler reduce la inversion inicial y permite asignar capital a materiales, personal y avance de obra.\n\nTambien facilita elegir el equipo adecuado para cada tarea sin asumir almacenamiento, depreciacion y mantenimiento de una flota propia.\n\nUna buena planificacion de fechas, cantidades y logistica ayuda a aprovechar al maximo cada periodo de alquiler.",
            'Principales ventajas de alquilar equipos para construccion.',
        ],
        [
            'atex-casetones-losas-alivianadas',
            'Atex: casetones para losas alivianadas',
            'Una introduccion al uso de casetones Atex y a la importancia de calcular los m2 de losa.',
            ASSET_BASE . '/images/especializaciones-casetones.jpg',
            "Los casetones se utilizan para reducir el peso propio de determinadas losas y optimizar el consumo de materiales segun el diseno estructural.\n\nPara preparar una cotizacion es fundamental conocer los metros cuadrados de losa. Ese dato permite estimar cantidades, rotacion y necesidades logisticas.\n\nLa definicion final siempre debe respetar los planos y criterios del profesional responsable de la estructura.",
            'Casetones Atex para losas alivianadas y calculo de metros cuadrados.',
        ],
    ];
    $stmt = db()->prepare(
...
function layout(string $title, string $content, string $active = '', string $description = '', string $image = ''): void
{
    ...
    $metaImage = absolute_url(trim($image) !== '' ? $image : DEFAULT_META_IMAGE);
...
function site_header(bool $showNavBar = true, bool $isAdmin = false): string
{
  ...
  <section class="nav-barra">
    ...
    <div class="div-block">
      ...
      <a href="/blog" class="enlace-barra-central w-inline-block"><div class="enlace-navbar">Blog</div></a>
...
<div class="header">
  <div class="fondo-nav" id="mobile-menu" aria-hidden="true">
    <div class="contendor-menu-movil">
      ...
      <a href="/blog" class="enlace-barra-central menu-movil w-inline-block"><div class="enlace-navbar">Blog</div></a>
...
function home_page(): void
{
    ...
    $heroImage = app_setting('hero_image', ASSET_BASE . '/images/imagen-banner-central.avif');
    $offerPopup = $offers !== [] && app_setting('offer_popup_enabled', '1') === '1'
        ? '<aside class="app-offer-popup" data-offer-popup data-confetti="' . h(app_setting('offer_confetti_enabled', '1')) . '" aria-label="Oferta"><button type="button" data-close-offer-popup aria-label="Cerrar">×</button><strong>Ofertas activas</strong><p>' . h(app_setting('offer_popup_message')) . '</p><a href="/productos-oferta">Ver ofertas</a></aside>'
        : '';
...
' . product_section('Productos novedades', $newProducts, 'productos-destacados', 'todas las novedades', null, '/productos-novedades') . $offerPopup;
...
{
    $cards = '';
    foreach (blog_posts(true) as $post) {
        $cards .= '<article class="app-blog-card"><a href="/blog/' . h($post['slug']) . '"><img src="' . h($post['image_url'] ?: DEFAULT_META_IMAGE) . '" alt="' . h($post['title']) . '"><div><time>' . h(substr((string) $post['published_at'], 0, 10)) . '</time><h2>' . h($post['title']) . '</h2><p>' . h($post['summary']) . '</p><span>Leer articulo</span></div></a></article>';
    }
...
function blog_article_page(string $slug): void
{
    ...
    layout((string) $post['title'], '<main class="app-blog-article"><a href="/blog">← Volver al blog</a><article><time>' . h(substr((string) $post['published_at'], 0, 10)) . '</time><h1>' . h($post['title']) . '</h1><p class="app-blog-summary">' . h($post['summary']) . '</p><img src="' . h($post['image_url'] ?: DEFAULT_META_IMAGE) . '" alt="' . h($post['title']) . '"><div class="app-blog-content">' . $paragraphs . '</div></article></main>', 'blog', (string) ($post['meta_description'] ?: $post['summary']), (string) ($post['image_url'] ?: DEFAULT_META_IMAGE));
}
...
function admin_settings_page(): void
{
    ...
    $values = [
        ...
        'hero_image' => app_setting('hero_image', ASSET_BASE . '/images/imagen-banner-central.avif'),
