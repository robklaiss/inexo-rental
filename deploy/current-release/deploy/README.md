...
- Extensiones PHP: `pdo_sqlite`, `sqlite3`, `json`, `mbstring`, `iconv`, `fileinfo`, `openssl`, `session`, `curl`.
...
- `INEXO_SMTP_PASSWORD`: clave SMTP usada por toda la aplicacion, incluido el cron de recordatorios.
- `INEXO_CONTACT_EMAIL`: destino de formularios de contacto.
- `INEXO_GOOGLE_MAPS_BROWSER_KEY`: clave restringida para Maps JavaScript/Places en el checkout.
- `INEXO_GOOGLE_ROUTES_API_KEY`: clave de servidor restringida a Routes API para `computeRoutes`.

El cron no tiene credenciales SMTP propias. `scripts/run_order_reminders.sh` entra al directorio de la aplicacion y PHP carga el mismo `.env` junto a `index.php` que usa produccion. Si el SMTP esta definido exclusivamente en el entorno de PHP-FPM, mover esas variables al `.env` protegido o exponerlas tambien al usuario de cron.

## Configuracion operativa

Crear una copia privada de `operations.example.json`, reemplazar todos los marcadores y ceros, y aplicarla desde la raiz de la aplicacion:
...
- direccion o coordenadas de origen de Inexo;
- capacidad maxima en kg de cada tipo de camion;
- capacidad maxima en m3;
- costo por km;
- descripcion y estado activo.

No guardar el archivo con datos internos dentro del repositorio.

Para comprobar que el CLI ve el SMTP productivo ya configurado:
...
La plantilla `cron.d/inexo-rental` ejecuta el proceso cada cinco minutos con bloqueo para evitar ejecuciones simultaneas. En un VPS con la ruta propuesta:

```bash
chmod +x scripts/run_order_reminders.sh
sudo cp deploy/cron.d/inexo-rental /etc/cron.d/inexo-rental
sudo chmod 644 /etc/cron.d/inexo-rental
```

Si la aplicacion, PHP o el usuario web usan otras rutas, ajustar la plantilla antes de copiarla. Verificar luego:

```bash
sudo -u www-data /var/www/inexo-rental/current/scripts/run_order_reminders.sh
tail -n 50 /var/log/inexo-rental-reminders.log
```
...
- `php scripts/check_operational_config.php` informa `"ready": true`.
- El cron de recordatorios esta instalado y se ejecuta con el mismo entorno SMTP de produccion.
