...
const DOCUMENT_UPLOAD_DIR = __DIR__ . '/data/private_documents';
const DOCUMENT_UPLOAD_BASE = '/data/private_documents';
const CONTENT_UPLOAD_DIR = __DIR__ . '/uploads/content';
const CONTENT_UPLOAD_BASE = '/uploads/content';
...
const DEFAULT_HERO_IMAGE = CONTENT_UPLOAD_BASE . '/hero-home-chatgpt-jun-17-2026.png';
...
const DEFAULT_BLOG_SEED_VERSION = 2;
...
function versioned_public_path(string $path): string
{
    ...
    return public_path($path) . $separator . 'v=' . rawurlencode($version);
}
...
function init_db(): void
{
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS products (
            ...
            requires_area_m2 INTEGER NOT NULL DEFAULT 0,
            deleted_at TEXT,
    ...
    ensure_column('products', 'weight_kg', 'REAL NOT NULL DEFAULT 0');
    ensure_column('products', 'volume_m3', 'REAL NOT NULL DEFAULT 0');
    ensure_column('products', 'is_stackable', 'INTEGER NOT NULL DEFAULT 0');
    ensure_column('products', 'requires_area_m2', 'INTEGER NOT NULL DEFAULT 0');
    $pdo->exec("UPDATE products SET requires_area_m2 = 1 WHERE lower(name) LIKE '%atex%' OR lower(slug) LIKE '%atex%'");
    ...
    ensure_column('orders', 'tax_name', "TEXT NOT NULL DEFAULT ''");
    ensure_column('orders', 'tax_rate', 'REAL NOT NULL DEFAULT 0');
    ensure_column('orders', 'tax_amount', 'REAL NOT NULL DEFAULT 0');
    ensure_column('orders', 'total_amount', 'REAL NOT NULL DEFAULT 0');
    ensure_column('orders', 'fulfillment_type', "TEXT NOT NULL DEFAULT 'delivery'");
    ensure_column('orders', 'scheduled_at', 'TEXT');
    ensure_column('orders', 'reminder_sent_at', 'TEXT');
    ...
    ensure_column('freight_truck_types', 'max_weight_kg', 'REAL NOT NULL DEFAULT 0');
    ensure_column('freight_truck_types', 'max_volume_m3', 'REAL NOT NULL DEFAULT 0');
    ensure_column('freight_truck_types', 'description', "TEXT NOT NULL DEFAULT ''");
    ...
    $pdo->exec(
        "CREATE TABLE IF NOT EXISTS shipments (
            ...
            origin_lat TEXT NOT NULL DEFAULT '',
            origin_lng TEXT NOT NULL DEFAULT '',
            destination_address TEXT NOT NULL DEFAULT '',
            destination_lat TEXT NOT NULL DEFAULT '',
            destination_lng TEXT NOT NULL DEFAULT '',
            route_url TEXT NOT NULL DEFAULT '',
            route_status TEXT NOT NULL DEFAULT 'pending_recalculation',
            distance_meters INTEGER NOT NULL DEFAULT 0,
            duration_seconds INTEGER NOT NULL DEFAULT 0,
            duration_traffic_seconds INTEGER NOT NULL DEFAULT 0,
            ...
            has_non_stackable INTEGER NOT NULL DEFAULT 0,
            accepted_at TEXT,
            completed_at TEXT,
            created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY(order_id) REFERENCES orders(id),
            FOREIGN KEY(truck_type_id) REFERENCES freight_truck_types(id),
            FOREIGN KEY(assigned_company_id) REFERENCES logistics_companies(id)
        )"
    );
    ensure_column('shipments', 'route_status', "TEXT NOT NULL DEFAULT 'pending_recalculation'");
    ensure_column('shipments', 'distance_meters', 'INTEGER NOT NULL DEFAULT 0');
    ensure_column('shipments', 'duration_seconds', 'INTEGER NOT NULL DEFAULT 0');
    ensure_column('shipments', 'duration_traffic_seconds', 'INTEGER NOT NULL DEFAULT 0');
    ensure_column('shipments', 'encoded_polyline', "TEXT NOT NULL DEFAULT ''");
    ensure_column('shipments', 'route_error', "TEXT NOT NULL DEFAULT ''");
    ensure_column('shipments', 'route_calculated_at', 'TEXT');
    $pdo->exec(
...
function app_setting_defaults(): array
{
    return [
        ...
        'offer_popup_message' => 'Descubri nuestras ofertas exclusivas por tiempo limitado.',
        'offer_confetti_enabled' => '1',
    ];
}
...
function default_blog_posts(): array
{
    return [
        [
            'slug' => 'inexo-rental-alquiler-de-equipos',
            'title' => 'Inexo Rental: equipos listos para cada etapa de tu obra',
            'summary' => 'Conoce como Inexo Rental facilita el acceso a equipos confiables para construccion.',
            'image_url' => ASSET_BASE . '/images/trabajadores-de-obra.avif',
            'content' => "Contar con el equipo correcto en el momento correcto evita atrasos, sobrecostos y decisiones improvisadas en plena obra. Cuando un frente de trabajo se detiene porque falta una maquina, el impacto no se limita al alquiler: tambien se frenan cuadrillas, entregas y compromisos de calendario.\n\nInexo Rental acompana proyectos de construccion con una seleccion de equipos para movimiento de suelo, compactacion, corte, energia, acceso y altura. La idea no es solo poner una maquina a disposicion, sino facilitar que cada etapa del proyecto tenga una respuesta operativa clara y realista.\n\nEso permite que el equipo tecnico o administrativo concentre su presupuesto en lo que realmente hace avanzar la obra. En lugar de inmovilizar capital en activos que quizas se usen de forma puntual, el alquiler ayuda a adaptar el equipamiento a la necesidad concreta de cada semana o de cada fase.\n\nTambien mejora la planificacion. Cuando la disponibilidad, el periodo de uso y la logistica se coordinan con anticipacion, la obra gana ritmo y previsibilidad. Un alquiler bien organizado reduce tiempos muertos y evita tener equipos parados mas dias de lo necesario.\n\nNuestro proceso permite solicitar alquileres, organizar la entrega o el retiro y recibir una proforma clara antes de confirmar. Esa claridad es importante porque ayuda a comparar opciones, validar cantidades y alinear expectativas entre compras, obra y contratistas.\n\nEn proyectos pequenos, medianos o de mayor escala, trabajar con un proveedor que entienda la dinamica del sitio puede marcar una diferencia concreta. La prioridad es que cada cliente pueda resolver una necesidad puntual sin agregar complejidad innecesaria.\n\nPor eso el servicio no gira solo alrededor del catalogo, sino alrededor de la continuidad de la obra. Tener acceso rapido a equipos adecuados, con informacion clara y una coordinacion simple, es parte de lo que permite que un proyecto siga avanzando sin perder traccion.",
            'meta_description' => 'Alquiler de equipos para construccion con Inexo Rental.',
        ],
        [
            'slug' => 'ventajas-alquiler-equipos-construccion',
            'title' => 'Ventajas del alquiler de equipos para construccion',
            'summary' => 'Por que alquilar puede mejorar el flujo de caja, la productividad y la planificacion.',
            'image_url' => ASSET_BASE . '/images/especializaciones-movimiento-de-suelo.jpg',
            'content' => "El alquiler reduce la inversion inicial y permite asignar capital a materiales, personal, transporte y avance de obra. Para muchas empresas, esa flexibilidad financiera es una de las decisiones mas importantes porque evita comprometer recursos en equipos que no van a trabajar de forma continua durante todo el ano.\n\nTambien facilita elegir el equipo adecuado para cada tarea sin asumir almacenamiento, depreciacion y mantenimiento de una flota propia. En construccion, una misma obra puede necesitar soluciones muy distintas entre excavacion, compactacion, elevacion, corte o soporte electrico, y no siempre conviene comprar para cubrir todas esas variaciones.\n\nOtro beneficio practico es la escalabilidad. Cuando aumenta el volumen de trabajo o aparece una etapa con mayor exigencia, alquilar permite sumar capacidad sin pasar por procesos largos de compra, aprobacion o incorporacion de activos. Eso da margen para responder mas rapido a oportunidades o cambios de cronograma.\n\nDesde el punto de vista operativo, el alquiler tambien ayuda a reducir cargas indirectas. Menos activos propios significa menos espacio ocupado, menos recursos dedicados a conservar equipos ociosos y menos preocupacion por la obsolescencia de ciertas maquinas frente a nuevas necesidades del mercado.\n\nUna buena planificacion de fechas, cantidades y logistica ayuda a aprovechar al maximo cada periodo de alquiler. Definir con precision cuando entra el equipo, cuantos dias se va a usar y si se retira o se entrega en obra evita pagar tiempo improductivo y mejora la coordinacion entre proveedores y responsables del frente de trabajo.\n\nAdemas, alquilar puede mejorar la lectura real de costos por proyecto. En vez de distribuir una compra grande durante meses o anos, el costo del equipo queda mas directamente asociado a la obra que lo necesita, lo que simplifica el control y la toma de decisiones.\n\nEn resumen, el alquiler no solo es una alternativa para reducir gasto inicial. Bien utilizado, es una herramienta de gestion que da flexibilidad financiera, mas capacidad de respuesta y una estructura operativa mas liviana para construir con mayor control.",
            'meta_description' => 'Principales ventajas de alquilar equipos para construccion.',
        ],
        [
            'slug' => 'atex-casetones-losas-alivianadas',
            'title' => 'Atex: casetones para losas alivianadas',
            'summary' => 'Una introduccion al uso de casetones Atex y a la importancia de calcular los m2 de losa.',
            'image_url' => ASSET_BASE . '/images/especializaciones-casetones.jpg',
            'content' => "Los casetones se utilizan para reducir el peso propio de determinadas losas y optimizar el consumo de materiales segun el diseno estructural. En sistemas de losas alivianadas, su uso forma parte de una solucion tecnica pensada para mejorar el comportamiento del conjunto sin cargar innecesariamente la estructura.\n\nCuando se trabaja con Atex u otros casetones similares, el punto de partida siempre debe ser el criterio estructural definido en planos. Las dimensiones, separaciones, apoyos y espesores no se eligen de forma aislada, sino en funcion del calculo realizado por el profesional responsable.\n\nPara preparar una cotizacion es fundamental conocer los metros cuadrados de losa. Ese dato permite estimar cantidades, rotacion y necesidades logisticas. Sin esa referencia, cualquier presupuesto corre el riesgo de quedarse corto o de incluir un margen excesivo que distorsione la decision de compra o alquiler.\n\nTambien conviene considerar desde el inicio la secuencia de obra. Saber en que frente se va a utilizar el material, durante cuanto tiempo y con que ritmo de avance ayuda a ordenar entregas y a evitar movimientos innecesarios en el sitio. La logistica influye tanto en el costo como en la productividad del sistema.\n\nEn proyectos con varias etapas, una medicion clara por sectores puede ser mas util que un unico numero global. Dividir la losa por paños o niveles permite coordinar mejor el uso del material, anticipar reposiciones y mantener una lectura mas fina del avance real.\n\nLa definicion final siempre debe respetar los planos y criterios del profesional responsable de la estructura. Ninguna referencia comercial reemplaza la validacion tecnica del proyecto especifico, especialmente cuando cambian luces, cargas o condicionantes constructivas.\n\nPor eso, antes de pedir una propuesta, conviene reunir la mayor cantidad posible de informacion util: metros cuadrados, planos, etapas estimadas y condiciones de acceso. Cuanto mas claro sea ese punto de partida, mas precisa sera la cotizacion y mas ordenada podra ser la ejecucion en obra.",
            'meta_description' => 'Casetones Atex para losas alivianadas y calculo de metros cuadrados.',
        ],
    ];
}

function seed_blog_posts(): void
{
    $posts = default_blog_posts();
    $stmt = db()->prepare(
        'INSERT OR IGNORE INTO blog_posts (slug, title, summary, image_url, content, meta_description) VALUES (?, ?, ?, ?, ?, ?)'
    );
    ...
    $version = (int) app_setting('default_blog_seed_version', '0');
    if ($version >= DEFAULT_BLOG_SEED_VERSION) {
        return;
    }

    $update = db()->prepare(
        'UPDATE blog_posts SET title = ?, summary = ?, image_url = ?, content = ?, meta_description = ? WHERE slug = ?'
    );
    foreach ($posts as $post) {
        $update->execute([
            $post['title'],
    ...
    db()->prepare('INSERT INTO app_settings (name, value) VALUES (?, ?) ON CONFLICT(name) DO UPDATE SET value = excluded.value')
        ->execute(['default_blog_seed_version', (string) DEFAULT_BLOG_SEED_VERSION]);
}
...
{
    return [
        'name' => trim(app_setting('tax_name', 'ITBIS')) ?: 'Impuesto',
        'rate' => max(0.0, app_setting_float('tax_rate', 0)),
    ];
}

function google_maps_route_url(array $delivery): string
{
    $origin = trim((string) ($delivery['origin_lat'] ?? '')) !== '' && trim((string) ($delivery['origin_lng'] ?? '')) !== ''
        ? trim((string) $delivery['origin_lat']) . ',' . trim((string) $delivery['origin_lng'])
        : trim((string) ($delivery['origin_address'] ?? ''));
    $destination = trim((string) ($delivery['lat'] ?? '')) !== '' && trim((string) ($delivery['lng'] ?? '')) !== ''
        ? trim((string) $delivery['lat']) . ',' . trim((string) $delivery['lng'])
    ...
    return 'https://www.google.com/maps/dir/?api=1&origin='
        . rawurlencode($origin) . '&destination=' . rawurlencode($destination) . '&travelmode=driving';
}

function fulfillment_type_label(string $type): string
{
    return $type === 'pickup' ? 'Retiro por cuenta propia' : 'Entrega con logistica';
}

function shipment_status_label(string $status): string
{
    return [
        'route_pending' => 'Ruta pendiente de recalcular',
        'pending' => 'Pendiente',
        'offered' => 'Ofrecido',
        'taken' => 'Tomado',
        'assigned' => 'Asignado',
...
function route_status_label(string $status): string
{
    return [
        ...
        'pending_recalculation' => 'Ruta pendiente de recalcular',
        'not_applicable' => 'No aplica',
    ][$status] ?? $status;
}

function format_route_duration(int $seconds): string
{
    if ($seconds <= 0) {
        return '';
    }
    ...
    return trim(($hours > 0 ? $hours . ' h ' : '') . ($minutes > 0 ? $minutes . ' min' : ''));
}
...
function legal_document_types(): array
{
    return [
        ...
        'tax_clearance' => 'Certificado de impuestos al dia',
        'tax_certificate' => 'Certificado tributario',
        'bank_certificate' => 'Certificado de cuenta bancaria',
    ];
}
...
function product_detail_lines(array $details): array
{
    if (($details['kind'] ?? '') !== 'product') {
        return [];
    }
    $area = max(0.0, (float) ($details['area_m2'] ?? 0));

    return $area > 0 ? ['Superficie de losa: ' . labor_number($area) . ' m²'] : [];
}
...
function google_routes_api_key(): string
{
    $serverKey = env_value([
    ...
    return $serverKey !== '' ? $serverKey : google_maps_browser_key();
}

function valid_lat_lng(mixed $latitude, mixed $longitude): ?array
{
...
function google_route_duration_seconds(mixed $duration): int
{
    if (!is_string($duration) || !preg_match('/^([0-9]+(?:\.[0-9]+)?)s$/', $duration, $match)) {
        return 0;
    }
...
function compute_google_route(array $origin, array $destination): array
{
    ...
    if ($apiKey === '') {
        return $failure('Google Routes API no tiene una clave de servidor configurada.');
    }
    if (!function_exists('curl_init')) {
        return $failure('La extension cURL no esta disponible en el servidor.');
    }
    ...
    if ($requestBody === false) {
        return $failure('No se pudo preparar la solicitud de ruta.');
    }
    ...
    if (!is_string($responseBody) || $responseBody === '') {
        return $failure('Google Routes API no respondio' . ($curlError !== '' ? ': ' . $curlError : '.'));
    }
    $response = json_decode($responseBody, true);
    if ($statusCode < 200 || $statusCode >= 300) {
        $apiMessage = trim((string) ($response['error']['message'] ?? ''));
        return $failure('Google Routes API respondio HTTP ' . $statusCode . ($apiMessage !== '' ? ': ' . $apiMessage : '.'));
    }
    $route = is_array($response['routes'][0] ?? null) ? $response['routes'][0] : [];
    $distanceMeters = max(0, (int) ($route['distanceMeters'] ?? 0));
    if ($distanceMeters <= 0) {
        return $failure('Google Routes API no devolvio una ruta valida.');
    }
    $trafficDuration = google_route_duration_seconds($route['duration'] ?? '');
    $staticDuration = google_route_duration_seconds($route['staticDuration'] ?? '');

    return [
        'status' => 'calculated',
        'distance_meters' => $distanceMeters,
        'duration_seconds' => $staticDuration > 0 ? $staticDuration : $trafficDuration,
        'duration_traffic_seconds' => $trafficDuration,
...
function admin_nav(string $active = 'productos'): string
{
    $links = [
        ...
        'logistica' => ['/admin/logistica', 'Logistica'],
...
function order_statuses(): array
{
    return [
        ...
        'ruta_pendiente_recalcular' => 'Ruta pendiente de recalcular',
        'validado' => 'Validado',
...
function send_order_customer_email(array $order, array $items, string $subject, string $message): bool
{
    ...
    foreach ($items as $item) {
        ...
        $savedDetails = order_item_details($item);
        $detailParts = array_merge($detailParts, labor_detail_lines($savedDetails), product_detail_lines($savedDetails));
        $rows .= '
          <tr>
...
function send_order_reminder_email(array $order): bool
{
    $scheduledAt = trim((string) ($order['scheduled_at'] ?? ''));
    if ($scheduledAt === '') {
        return false;
    }
    $fulfillment = (string) ($order['fulfillment_type'] ?? 'delivery');
    $modeLabel = fulfillment_type_label($fulfillment);
    $message = $fulfillment === 'pickup'
        ? 'Te recordamos que el retiro de tu pedido esta coordinado para el ' . $scheduledAt . '.'
        : 'Te recordamos que el flete de tu pedido esta coordinado para el ' . $scheduledAt . '.';

    return send_order_customer_email(
        $order,
        order_items((int) $order['id']),
        'Recordatorio de ' . strtolower($modeLabel) . ' - pedido #' . (int) $order['id'],
        $message
    );
}

function process_due_order_reminders(): array
{
    $orders = db()->query(
        "SELECT orders.*, users.email, users.name
        FROM orders
        JOIN users ON users.id = orders.user_id
        WHERE orders.scheduled_at IS NOT NULL
          AND orders.reminder_sent_at IS NULL
          AND orders.status NOT IN ('cancelado', 'finalizado')
          AND datetime(orders.scheduled_at) <= datetime('now', '+24 hours')
          AND datetime(orders.scheduled_at) > datetime('now')
        ORDER BY orders.scheduled_at ASC"
    )->fetchAll();
    $sent = 0;
    $failed = 0;
    foreach ($orders as $order) {
        if (send_order_reminder_email($order)) {
            db()->prepare("UPDATE orders SET reminder_sent_at = datetime('now') WHERE id = ? AND reminder_sent_at IS NULL")->execute([(int) $order['id']]);
            $sent++;
        } else {
            $failed++;
        }
    ...
    return ['due' => count($orders), 'sent' => $sent, 'failed' => $failed];
}
...
function layout(string $title, string $content, string $active = '', string $description = '', string $image = ''): void
{
    $isAdmin = $active === 'admin';
    $fullTitle = $title . ' - Inexo Rental';
    $metaDescription = trim($description) !== '' ? trim($description) : DEFAULT_META_DESCRIPTION;
    $metaImage = absolute_url(versioned_public_path(trim($image) !== '' ? $image : DEFAULT_META_IMAGE));
...
function site_header(bool $showNavBar = true, bool $isAdmin = false): string
{
  ...
  <section class="nav-barra">
    ...
    <button class="dropdown-menu-movil app-mobile-toggle" type="button" data-open-menu>
      <div class="enlace-navbar">navegar</div><img src="' . ASSET_BASE . '/images/flecha-simple.png" loading="lazy" height="20" alt="Black and white downward arrow icon with rounded edges and a circle border." class="flecha-abajo">
    </button>
    <div class="div-block">
  ...
  <div class="contenedor-footer">
    ...
    <div class="contenedor-costado-nav-footer">
      <div class="nav-footer">
        ...
        <a href="/blog" class="enlace-barra-central w-inline-block"><div class="enlace-navbar blanco">Blog</div></a>
...
function product_card(array $product): string
{
    ...
    $offerCountdown = '';
    if ($isOffer) {
        $offerCountdown = '<div class="app-offer-badge">' . product_offer_countdown_html($product) . '</div>';
    }

    return '
<div class="w-layout-cell">
  <article class="card-producto' . ($isOffer ? ' app-product-offer' : '') . (!$isOrderable ? ' app-product-unavailable' : '') . '">
...
function home_page(): void
{
    ...
    $latestBlogPosts = array_slice(blog_posts(true), 0, 3);
    ...
    $heroImage = app_setting('hero_image', DEFAULT_HERO_IMAGE);
    $offerPopup = $offers !== [] && app_setting('offer_popup_enabled', '1') === '1'
        ? '<aside class="app-offer-popup" data-offer-popup data-confetti="' . h(app_setting('offer_confetti_enabled', '1')) . '" aria-label="Oferta"><button type="button" data-close-offer-popup aria-label="Cerrar">×</button><strong>Ofertas exclusivas</strong><p>' . h(app_setting('offer_popup_message')) . '</p><a href="/productos-oferta">Ver ofertas</a></aside>'
        : '';
...
<section class="banner-central">
  <div class="w-layout-layout quick-stack wf-layout-layout app-hero-grid">
    ...
    <div class="w-layout-cell"><img src="' . h($heroImage) . '" loading="eager" alt="Operador de construccion" class="contenedor-derecho app-configurable-hero"></div>
  </div>
</section>
...
' . ($latestBlogPosts !== [] ? blog_section('Ideas y recursos para tu obra', $latestBlogPosts, 'inicio', 'todos los articulos', '/blog') : '') . $offerPopup;

    layout('Inicio', $content, 'inicio');
}

function blog_posts(bool $publishedOnly = true): array
{
...
function blog_cards_html(array $posts): string
{
    $cards = '';
    foreach ($posts as $post) {
        $cards .= '<article class="app-blog-card"><a href="/blog/' . h($post['slug']) . '" class="app-blog-card-link"><img src="' . h($post['image_url'] ?: DEFAULT_META_IMAGE) . '" alt="' . h($post['title']) . '"><div><time>' . h(substr((string) $post['published_at'], 0, 10)) . '</time><h2>' . h($post['title']) . '</h2><p>' . h($post['summary']) . '</p><span>Leer articulo</span></div></a></article>';
    }

    return $cards;
}

function blog_section(string $title, array $posts, string $className = 'app-blog-section', string $cta = '', string $ctaHref = '/blog'): string
{
    $cards = blog_cards_html($posts);
    if ($cards === '') {
        return '';
    }

    return '
<section class="' . h($className) . ' app-blog-section">
' . listing_top_bar($title, null, $cta, $ctaHref) . '
  <div class="app-blog-grid">' . $cards . '</div>
</section>';
}

function blog_page(): void
{
    $posts = blog_posts(true);
    $cards = blog_cards_html($posts);
    layout('Blog', '<main class="app-admin-shell app-blog-page"><div class="app-admin-header"><div><p class="app-kicker">Blog</p><h1>Ideas y recursos para tu obra</h1></div></div><section class="app-blog-grid">' . ($cards ?: '<p>No hay articulos publicados.</p>') . '</section></main>', 'blog');
}

function blog_article_page(string $slug): void
{
    ...
    $paragraphs = '';
    foreach (preg_split('/\R{2,}/', trim((string) $post['content'])) ?: [] as $paragraph) {
        $paragraphs .= '<p>' . nl2br(h($paragraph)) . '</p>';
    }
    layout((string) $post['title'], '<main class="app-blog-article"><article><time>' . h(substr((string) $post['published_at'], 0, 10)) . '</time><h1>' . h($post['title']) . '</h1><p class="app-blog-summary">' . h($post['summary']) . '</p><img src="' . h($post['image_url'] ?: DEFAULT_META_IMAGE) . '" alt="' . h($post['title']) . '"><div class="app-blog-content">' . $paragraphs . '</div><a href="/blog" class="button w-button app-blog-back-link">← Volver al blog</a></article></main>', 'blog', (string) ($post['meta_description'] ?: $post['summary']), (string) ($post['image_url'] ?: DEFAULT_META_IMAGE));
}
...
function detail_page(string $slug): void
{
    ...
    if ((string) ($product['product_type'] ?? 'standard') === 'labor') {
      ...
      <form class="app-price-card app-labor-card" data-labor-form>
        ...
        $areaRequirement = (int) ($product['requires_area_m2'] ?? 0) === 1
            ? '<label class="app-field-label" for="area_m2">¿Para cuantos m² de losa necesitas casetones?</label><input id="area_m2" name="area_m2" type="number" min="0.01" step="0.01" required placeholder="Ej. 250">'
            : '';
        $commerceCards = '
    ...
    <aside class="app-product-panel">
      ...
      <form class="app-rental-card' . ((string) ($product['product_type'] ?? 'standard') === 'labor' ? ' app-hidden' : '') . '" data-reservation-form>
        ...
        <div class="app-rental-body">
          ...
          ' . $areaRequirement . '
          <button class="app-reserve-button" type="submit" data-product-id="' . (int) $product['id'] . '" data-product-name="' . h($product['name']) . '" data-product-url="/producto/' . h($product['slug']) . '" data-product-image="' . $productImage . '" data-requires-area-m2="' . (int) ($product['requires_area_m2'] ?? 0) . '"' . $disabledAttr . '>Reserve equipo ahora</button>
...
<main class="app-admin-shell app-checkout-shell">
  ...
  <div class="app-checkout-grid">
    ...
    <form class="app-admin-form app-checkout-form" data-checkout-form>
      ' . $accountFields . '
      <h2 class="app-form-wide">¿Como queres recibir el producto?</h2>
      <div class="app-form-wide app-fulfillment-options">
        <label><input type="radio" name="fulfillment_type" value="pickup" required><span><strong>Retiro por cuenta propia</strong><small>Retiras el pedido en Inexo. No se calcula flete ni se solicita pin.</small></span></label>
        <label><input type="radio" name="fulfillment_type" value="delivery" required><span><strong>Entrega con logistica</strong><small>Completa la ubicacion y los datos necesarios para coordinar el envio.</small></span></label>
      </div>
      <label class="app-form-wide">Fecha y hora de entrega o retiro<input name="scheduled_at" type="datetime-local" required></label>
      <section class="app-form-wide app-logistics-fields app-hidden" data-logistics-fields>
        <div class="app-admin-form app-nested-form">
          <h2 class="app-form-wide">Datos de entrega</h2>
          <label>Destino<select name="delivery_type" data-logistics-required><option value="">Seleccionar destino</option><option value="empresa">Entrega en Empresa</option><option value="obra">Entrega en Obra</option></select></label>
          <label>Ciudad<select name="city" data-delivery-city data-logistics-required><option value="">Seleccionar ciudad</option>' . $cities . '</select></label>
          ...
          <label class="app-form-wide">Direccion de entrega<input name="address" data-delivery-address data-logistics-required autocomplete="off" placeholder="Escribir ciudad, sector o direccion"></label>
          ...
          <div class="app-form-wide app-map-callout">El pin es obligatorio porque define el destino de la ruta logistica.</div>
          <label>Nombre recibe<input name="delivery_contact_first_name" data-logistics-required></label>
          <label>Apellido recibe<input name="delivery_contact_last_name" data-logistics-required></label>
          <label>Telefono de oficina<input name="delivery_office_phone" data-logistics-required></label>
          <label>Movil responsable<input name="delivery_mobile_phone" data-logistics-required></label>
          ...
          <div class="app-form-wide app-freight-box" data-freight-calculator data-origin-address="' . h($freight['origin_address']) . '" data-origin-lat="' . h($freight['origin_lat']) . '" data-origin-lng="' . h($freight['origin_lng']) . '" data-round-trip-factor="' . h($freight['round_trip_factor']) . '" data-tax-name="' . h($tax['name']) . '" data-tax-rate="' . h($tax['rate']) . '">
            <label>Tipo de camion<select name="freight_truck_type_id" data-truck-type data-logistics-required><option value="">Seleccionar camion</option>' . $truckOptions . '</select></label>
            <label>Distancia ida estimada en km<input name="delivery_distance_km" type="number" min="0.01" step="0.01" data-distance-km readonly placeholder="La ruta final se calcula en el servidor"></label>
            <div class="app-freight-summary" data-freight-summary>Flete por confirmar</div>
          </div>
        </div>
      </section>
      <div class="app-form-wide app-tax-note">La proforma aplicara ' . h($tax['name']) . ' (' . h(labor_number($tax['rate'])) . '%) sobre equipos y flete.</div>
...
function account_page(): void
{
    ...
    $documentStmt = db()->prepare('SELECT * FROM user_documents WHERE user_id = ?');
    $documentStmt->execute([(int) $user['id']]);
    $savedDocuments = [];
    foreach ($documentStmt->fetchAll() as $document) {
        $savedDocuments[(string) $document['document_type']] = $document;
    }
    $documentCards = '';
    foreach (legal_document_types() as $type => $label) {
        $document = $savedDocuments[$type] ?? null;
        $status = $document
            ? '<span class="is-complete">Cargado ' . h((string) $document['uploaded_at']) . '</span><a href="/cuenta/documentos/' . (int) $document['id'] . '">Ver o descargar</a>'
            : '<span class="is-missing">Pendiente</span>';
        $documentCards .= '<form action="/cuenta/documentos" method="post" enctype="multipart/form-data" class="app-document-card"><input type="hidden" name="document_type" value="' . h($type) . '"><strong>' . h($label) . '</strong>' . $status . '<input name="document_file" type="file" accept="application/pdf,image/jpeg,image/png" required><button type="submit">' . ($document ? 'Reemplazar' : 'Subir documento') . '</button></form>';
    }
...
<main class="app-admin-shell">
  ...
  <section class="app-account-documents">
    <h2>Documentos legales</h2>
    <p>Formatos permitidos: PDF, JPG y PNG. Tamano maximo: 10 MB por archivo.</p>
    <div class="app-document-grid">' . $documentCards . '</div>
  </section>
</main>', 'cuenta');
}

function save_user_document(): void
{
    $user = current_user();
    if (!$user) {
        redirect_to('/ingresar');
    ...
    if (!array_key_exists($type, legal_document_types())) {
        redirect_to('/cuenta');
    }
    $file = $_FILES['document_file'] ?? null;
    if (!is_array($file) || (int) ($file['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK || (int) ($file['size'] ?? 0) > MAX_LEGAL_DOCUMENT_BYTES) {
        redirect_to('/cuenta?documento=error');
    }
    ...
    $userDir = DOCUMENT_UPLOAD_DIR . '/' . (int) $user['id'];
    if (!is_dir($userDir)) {
        mkdir($userDir, 0770, true);
    }
    $filename = $type . '-' . bin2hex(random_bytes(8)) . '.' . $allowed[$mime];
    $destination = $userDir . '/' . $filename;
    if (!move_uploaded_file($tmp, $destination)) {
        redirect_to('/cuenta?documento=error');
    }
    $storedPath = DOCUMENT_UPLOAD_BASE . '/' . (int) $user['id'] . '/' . $filename;
    $existing = db()->prepare('SELECT stored_path FROM user_documents WHERE user_id = ? AND document_type = ?');
    $existing->execute([(int) $user['id'], $type]);
    $oldPath = (string) ($existing->fetchColumn() ?: '');
    db()->prepare(
        "INSERT INTO user_documents (user_id, document_type, original_name, stored_path, mime_type, file_size, uploaded_at)
        VALUES (?, ?, ?, ?, ?, ?, datetime('now'))
        ON CONFLICT(user_id, document_type) DO UPDATE SET
            original_name = excluded.original_name, stored_path = excluded.stored_path,
            mime_type = excluded.mime_type, file_size = excluded.file_size, uploaded_at = datetime('now')"
    )->execute([(int) $user['id'], $type, basename((string) $file['name']), $storedPath, $mime, (int) $file['size']]);
    if ($oldPath !== '' && str_starts_with($oldPath, DOCUMENT_UPLOAD_BASE . '/')) {
        $oldFile = __DIR__ . $oldPath;
        if (is_file($oldFile) && $oldFile !== $destination) {
            @unlink($oldFile);
        }
    }
    redirect_to('/cuenta?documento=ok');
}

function output_user_document(int $documentId, bool $admin = false): void
{
    $user = current_user();
    $stmt = db()->prepare('SELECT * FROM user_documents WHERE id = ?');
    $stmt->execute([$documentId]);
    $document = $stmt->fetch();
    if (!$document || (!$admin && (!$user || (int) $document['user_id'] !== (int) $user['id']))) {
        not_found();
        return;
    }
    $path = __DIR__ . (string) $document['stored_path'];
    if (!is_file($path)) {
        not_found();
        return;
    }
...
function admin_dashboard_page(): void
{
    ...
    $pendingOrders = admin_count('orders', "status IN ('pendiente_validacion', 'ruta_pendiente_recalcular')");
...
function admin_order_notice(): string
{
    ...
    if ($status === 'route_recalculated') {
        return '<div class="app-contact-feedback is-success">La ruta logistica se recalculo correctamente.</div>';
    }
    if ($status === 'route_failed') {
        return '<div class="app-contact-feedback is-error">La ruta sigue pendiente de recalcular. Revisa la clave, permisos y configuracion de Google Routes API.</div>';
    }
...
function admin_order_detail_page(int $orderId): void
{
    ...
    foreach ($items as $item) {
        ...
        $savedDetails = order_item_details($item);
        $detailText = implode(' - ', array_merge($details, labor_detail_lines($savedDetails), product_detail_lines($savedDetails)));
    ...
    foreach ([
        ...
        'distance_meters' => 'Distancia Routes API',
        'duration_seconds' => 'Duracion estimada sin trafico',
        'duration_traffic_seconds' => 'Duracion estimada con trafico',
        'route_status' => 'Estado de ruta',
        'route_calculated_at' => 'Ruta calculada el',
        'route_error' => 'Aviso interno de ruta',
        'route_polyline' => 'Polyline guardada',
        'freight_total' => 'Total flete calculado',
        'route_url' => 'Ruta Google Maps',
    ] as $key => $label) {
        ...
        if ($value !== '') {
            ...
            if ($key === 'route_url') {
                $displayValue = '<a href="' . h($value) . '" target="_blank" rel="noopener">Abrir recorrido</a>';
            } elseif ($key === 'distance_meters') {
                $displayValue = h(number_format(((int) $value) / 1000, 2, '.', ',') . ' km');
            } elseif (in_array($key, ['duration_seconds', 'duration_traffic_seconds'], true)) {
                $displayValue = h(format_route_duration((int) $value));
            } elseif ($key === 'route_status') {
                $displayValue = h(route_status_label($value));
            } elseif ($key === 'route_polyline') {
                $displayValue = h(strlen($value) . ' caracteres');
            } else {
                $displayValue = h($value);
            }
            $deliveryRows .= '<tr><th>' . h($label) . '</th><td>' . $displayValue . '</td></tr>';
        }
    }
    $routePendingNotice = ($deliverySnapshot['route_status'] ?? '') === 'pending_recalculation'
        ? '<div class="app-contact-feedback is-error">Ruta pendiente de recalcular. El pedido fue guardado, pero Google Routes API no completo el calculo.</div>'
        : '';
    $routeRecalculateForm = ($order['fulfillment_type'] ?? '') === 'delivery'
        ? '<form action="/admin/pedidos/' . (int) $order['id'] . '/recalcular-ruta" method="post" class="app-admin-form app-order-action-form">
        <h2 class="app-form-wide">Ruta logistica</h2>
        <p class="app-form-wide">Estado: ' . h(route_status_label((string) ($deliverySnapshot['route_status'] ?? 'pending_recalculation'))) . '</p>
        <div class="app-form-actions"><button class="btn-contacto w-button" type="submit">Recalcular ruta</button></div>
      </form>'
        : '';
    ...
    <div class="app-admin-card app-order-detail-main">
      ...
      <table class="app-order-meta-table">
        <tbody>
          ...
          <tr><th>' . h((string) ($order['tax_name'] ?? 'Impuesto')) . ' (' . h(labor_number((float) ($order['tax_rate'] ?? 0))) . '%)</th><td>' . money($order['tax_amount'] ?? 0) . '</td></tr>
...
function recalculate_order_route(int $orderId): void
{
    $order = order_by_id($orderId);
    if (!$order || (string) ($order['fulfillment_type'] ?? '') !== 'delivery') {
        not_found();
        return;
    }
    $shipmentStmt = db()->prepare('SELECT * FROM shipments WHERE order_id = ?');
    ...
    $origin = valid_lat_lng($shipment['origin_lat'] ?? null, $shipment['origin_lng'] ?? null);
    $destination = valid_lat_lng($shipment['destination_lat'] ?? null, $shipment['destination_lng'] ?? null);
    if (!$origin || !$destination) {
        redirect_to('/admin/pedidos/' . $orderId . '?status=route_failed');
    }

    $route = compute_google_route($origin, $destination);
    $delivery = json_decode((string) ($order['delivery_snapshot'] ?? '{}'), true) ?: [];
    $delivery['route_status'] = $route['status'];
    $delivery['route_error'] = $route['error'];
    $delivery['route_calculated_at'] = $route['calculated_at'];
    if ($route['status'] !== 'calculated') {
        $newShipmentStatus = (int) ($shipment['assigned_company_id'] ?? 0) > 0
            ? (string) $shipment['status']
            : 'route_pending';
        db()->prepare(
            'UPDATE shipments SET status = ?, route_status = ?, route_error = ?, route_calculated_at = NULL WHERE id = ?'
        )->execute([$newShipmentStatus, $route['status'], $route['error'], (int) $shipment['id']]);
        db()->prepare('UPDATE orders SET delivery_snapshot = ? WHERE id = ?')->execute([
    ...
    $distanceKm = round(((int) $route['distance_meters']) / 1000, 3);
    $roundTripFactor = max(0.0, app_setting_float('freight_round_trip_factor', DEFAULT_FREIGHT_ROUND_TRIP_FACTOR));
    $truckType = freight_truck_type_by_id((int) ($shipment['truck_type_id'] ?? 0));
    $freightAmount = calculate_freight_amount($distanceKm, $truckType);
    $taxRate = max(0.0, (float) ($order['tax_rate'] ?? 0));
    $taxableAmount = round((float) ($order['subtotal_amount'] ?? 0) + $freightAmount, 2);
    $taxAmount = round($taxableAmount * ($taxRate / 100), 2);
    $totalAmount = round($taxableAmount + $taxAmount, 2);
    $delivery = array_merge($delivery, [
        'distance_km_one_way' => $distanceKm,
        'distance_km_round_trip' => $distanceKm * $roundTripFactor,
        'distance_meters' => (int) $route['distance_meters'],
        'duration_seconds' => (int) $route['duration_seconds'],
        'duration_traffic_seconds' => (int) $route['duration_traffic_seconds'],
        'route_polyline' => (string) $route['encoded_polyline'],
        'route_status' => 'calculated',
        'route_error' => '',
        'route_calculated_at' => (string) $route['calculated_at'],
        'distance_source' => 'google_routes_api',
        'freight_total' => $freightAmount,
    ]);
    $activeLogistics = (int) db()->query('SELECT COUNT(*) FROM logistics_companies WHERE is_active = 1')->fetchColumn();
    $newShipmentStatus = (string) $shipment['status'] === 'route_pending'
        ? ($activeLogistics > 0 ? 'offered' : 'pending')
        : (string) $shipment['status'];
    ...
    try {
        $pdo->prepare(
            "UPDATE orders SET delivery_snapshot = ?, freight_amount = ?, tax_amount = ?, total_amount = ?,
                status = CASE WHEN status = 'ruta_pendiente_recalcular' THEN 'validado' ELSE status END
            WHERE id = ?"
        )->execute([
            json_encode($delivery, JSON_UNESCAPED_UNICODE),
            $freightAmount,
        ...
        $pdo->prepare(
            'UPDATE shipments SET status = ?, route_status = ?, distance_meters = ?, duration_seconds = ?,
                duration_traffic_seconds = ?, encoded_polyline = ?, route_error = ?, route_calculated_at = ?
            WHERE id = ?'
        )->execute([
            $newShipmentStatus,
            'calculated',
            ...
            (int) $route['duration_traffic_seconds'],
            (string) $route['encoded_polyline'],
            '',
            (string) $route['calculated_at'],
            (int) $shipment['id'],
        ]);
        $pdo->commit();
    ...
    redirect_to('/admin/pedidos/' . $orderId . '?status=route_recalculated');
}
...
function proforma_lines(array $order, array $items): array
{
    ...
    foreach ($items as $item) {
        ...
        foreach (product_detail_lines(order_item_details($item)) as $productLine) {
            $lines[] = '  ' . $productLine;
        }
    }
    ...
    $lines[] = (string) ($order['tax_name'] ?? 'Impuesto') . ' (' . labor_number((float) ($order['tax_rate'] ?? 0)) . '%): ' . money($order['tax_amount'] ?? 0);
...
function proforma_pdf_number(mixed $value): ?float
{
...
function proforma_pdf_delivery_lat_lng(array $delivery): ?array
{
    $lat = proforma_pdf_number($delivery['lat'] ?? null);
    $lng = proforma_pdf_number($delivery['lng'] ?? null);
...
function proforma_pdf_city_center(mixed $city): ?array
{
    ...
    $lookup = [
        ...
        'san pedro de macoris' => ['lat' => 18.4616, 'lng' => -69.2972],
        'san francisco de macoris' => ['lat' => 19.3000, 'lng' => -70.2526],
        'la vega' => ['lat' => 19.2221, 'lng' => -70.5296],
        'puerto plata' => ['lat' => 19.7808, 'lng' => -70.6871],
        'bonao' => ['lat' => 18.9369, 'lng' => -70.4092],
        'moca' => ['lat' => 19.3935, 'lng' => -70.5250],
        'bani' => ['lat' => 18.2796, 'lng' => -70.3319],
        'azua' => ['lat' => 18.4532, 'lng' => -70.7349],
        'barahona' => ['lat' => 18.2085, 'lng' => -71.1008],
        'higuey' => ['lat' => 18.6150, 'lng' => -68.7070],
        'san cristobal' => ['lat' => 18.4167, 'lng' => -70.1056],
    ];

    return $lookup[$normalized] ?? null;
}

function proforma_pdf_delivery_map_card(float $x, float $topY, float $width, float $height, array $delivery): string
{
    $cmd = '';
    $cmd .= proforma_pdf_rect($x, $topY - $height, $width, $height, '#f7f8f9', '#d8dcdf', 0.6);
    $cmd .= proforma_pdf_text($x + 12, $topY - 16, 'Mapa de entrega (pin marcado)', 'F2', 9, '#5d646b');
    $mapX = $x + 12.0;
    $mapY = $topY - 28.0;
    $mapW = $width - 24.0;
    $mapH = $height - 42.0;
    $cmd .= proforma_pdf_rect($mapX, $mapY - $mapH, $mapW, $mapH, '#e8edf1', '#cfd7dd', 0.4);

    for ($i = 1; $i <= 4; $i++) {
        $lineY = ($mapY - $mapH) + (($mapH / 5.0) * $i);
        $lineX = $mapX + (($mapW / 6.0) * $i);
        $cmd .= proforma_pdf_line($mapX + 6, $lineY, $mapX + $mapW - 6, $lineY, '#d4dde4', 0.3);
        $cmd .= proforma_pdf_line($lineX, $mapY - $mapH + 6, $lineX, $mapY - 6, '#d4dde4', 0.3);
    }
    $cmd .= proforma_pdf_line($mapX + 18, $mapY - $mapH + 22, $mapX + $mapW - 22, $mapY - $mapH + 54, '#c2ccd4', 1.0);
    $cmd .= proforma_pdf_line($mapX + 26, $mapY - $mapH + 86, $mapX + $mapW - 34, $mapY - $mapH + 64, '#c2ccd4', 1.0);

    $latLng = proforma_pdf_delivery_lat_lng($delivery);
    $pinSource = 'exacta';
    if (!$latLng) {
        $latLng = proforma_pdf_city_center($delivery['city'] ?? '');
        $pinSource = $latLng ? 'ciudad' : 'aproximada';
    }
    if (!$latLng) {
    ...
    if ($latLng) {
        ...
        $lngMin = -72.20;
        $lngMax = -68.10;
        $latRatio = ($latLng['lat'] - $latMin) / max(0.0001, ($latMax - $latMin));
        $lngRatio = ($latLng['lng'] - $lngMin) / max(0.0001, ($lngMax - $lngMin));
        $latRatio = min(1.0, max(0.0, $latRatio));
        $lngRatio = min(1.0, max(0.0, $lngRatio));
        $pinX = $mapX + ($lngRatio * $mapW);
        $pinY = ($mapY - $mapH) + ((1.0 - $latRatio) * $mapH);
        $cmd .= proforma_pdf_line($pinX - 8, $pinY, $pinX + 8, $pinY, '#d63447', 0.9);
        $cmd .= proforma_pdf_line($pinX, $pinY - 8, $pinX, $pinY + 8, '#d63447', 0.9);
        $cmd .= proforma_pdf_rect($pinX - 2.8, $pinY + 2.8, 5.6, 5.6, '#d63447', '#a92637', 0.3);
        $pinLabel = 'Pin exacto';
        if ($pinSource === 'ciudad') {
            $pinLabel = 'Pin aproximado por ciudad';
        } elseif ($pinSource === 'aproximada') {
            $pinLabel = 'Pin aproximado';
        }
        $cmd .= proforma_pdf_text($mapX + 10, $mapY - $mapH + 12, $pinLabel . ': ' . number_format($latLng['lat'], 6, '.', '') . ', ' . number_format($latLng['lng'], 6, '.', ''), 'F1', 8, '#5d646b');
    }
...
function proforma_pdf_logo_object(): ?array
{
    ...
    $paths = [
        __DIR__ . '/assets/inexo-rental-logo-pdf.png',
        __DIR__ . '/inexo-rental---tu-partner-en-cada-obra.webflow/images/inexo-pos.png',
        __DIR__ . '/inexo-rental---tu-partner-en-cada-obra.webflow/images/inexo-rental.png',
    ];
    $source = null;
...
function order_proforma_pretty_pdf_bytes(array $order, array $items): string
{
    ...
    $sectionTitle('Ubicacion de entrega');
    $latLng = proforma_pdf_delivery_lat_lng($delivery);
    $infoCard([
        'Direccion marcada' => $delivery['address'] ?? '',
        'Coordenadas' => $latLng ? number_format($latLng['lat'], 6, '.', '') . ', ' . number_format($latLng['lng'], 6, '.', '') : 'No registradas',
    ]);
    $mapHeight = 162.0;
    $ensureSpace($mapHeight + 12);
    $content .= proforma_pdf_delivery_map_card($margin, $y, $usableWidth, $mapHeight, $delivery);
    $y -= $mapHeight + 20.0;
    ...
    foreach ($items as $item) {
        ...
        $savedDetails = order_item_details($item);
        $detailLines = array_merge([implode(' / ', $details)], labor_detail_lines($savedDetails), product_detail_lines($savedDetails));
    ...
    $content .= proforma_pdf_rect($boxX, $y - 108, $boxW, 108, '#f7f8f9', '#d8dcdf', 0.6);
    ...
    $taxLabel = trim((string) ($order['tax_name'] ?? '')) ?: 'Impuesto';
    $taxLabel .= ' (' . labor_number((float) ($order['tax_rate'] ?? 0)) . '%)';
    $content .= proforma_pdf_text($boxX + 14, $y - 64, $taxLabel, 'F1', 9, '#5d646b');
    $content .= proforma_pdf_text($boxX + 135, $y - 64, money($order['tax_amount'] ?? 0), 'F1', 9, '#222222');
    $content .= proforma_pdf_line($boxX + 14, $y - 76, $boxX + $boxW - 14, $y - 76, '#d8dcdf', 0.6);
    $content .= proforma_pdf_text($boxX + 14, $y - 94, 'TOTAL', 'F2', 10, '#111111');
    $content .= proforma_pdf_text($boxX + 135, $y - 94, money($order['total_amount'] ?? 0), 'F2', 11, '#111111');
    $y -= 134;
...
function admin_settings_page(): void
{
    ...
    $values = [
        ...
        'tax_name' => app_setting('tax_name', 'ITBIS'),
        'tax_rate' => app_setting('tax_rate', '16'),
        'hero_image' => app_setting('hero_image', DEFAULT_HERO_IMAGE),
        'offer_popup_enabled' => app_setting('offer_popup_enabled', '1'),
        'offer_popup_message' => app_setting('offer_popup_message'),
        'offer_confetti_enabled' => app_setting('offer_confetti_enabled', '1'),
    ];
    $truckRows = '';
    foreach (freight_truck_types(false) as $truckType) {
      ...
      <tr>
        ...
        <td><input name="truck_types[' . $truckId . '][max_weight_kg]" type="number" step="0.01" min="0" value="' . h($truckType['max_weight_kg']) . '"></td>
        <td><input name="truck_types[' . $truckId . '][max_volume_m3]" type="number" step="0.001" min="0" value="' . h($truckType['max_volume_m3']) . '"></td>
        <td><input name="truck_types[' . $truckId . '][cost_per_km]" type="number" step="0.01" min="0" value="' . h($truckType['cost_per_km']) . '" required></td>
        <td><input name="truck_types[' . $truckId . '][description]" value="' . h($truckType['description']) . '"></td>
...
<main class="app-admin-shell">
  ...
  <form action="/admin/configuracion" method="post" class="app-admin-form" enctype="multipart/form-data">
    ' . $savedNotice . '
    <h2 class="app-form-wide">Proforma e impuestos</h2>
    <label>Nombre del impuesto<input name="tax_name" value="' . h($values['tax_name']) . '" required></label>
    <label>Porcentaje<input name="tax_rate" type="number" min="0" step="0.01" value="' . h($values['tax_rate']) . '" required></label>
    <h2 class="app-form-wide">Inicio y ofertas</h2>
    <label class="app-form-wide">Imagen Hero actual o URL<input name="hero_image" value="' . h($values['hero_image']) . '"></label>
    <label class="app-form-wide">Reemplazar imagen Hero<input name="hero_image_file" type="file" accept="image/jpeg,image/png,image/webp,image/avif"></label>
    <label class="app-check"><input type="checkbox" name="offer_popup_enabled" value="1" ' . ($values['offer_popup_enabled'] === '1' ? 'checked' : '') . '> Mostrar popup de ofertas</label>
    <label class="app-check"><input type="checkbox" name="offer_confetti_enabled" value="1" ' . ($values['offer_confetti_enabled'] === '1' ? 'checked' : '') . '> Confeti sutil en ofertas</label>
    <label class="app-form-wide">Mensaje del popup<input name="offer_popup_message" value="' . h($values['offer_popup_message']) . '"></label>
    ...
    <div class="app-form-wide app-admin-card app-inline-table-card">
      <table class="app-admin-table">
        <thead><tr><th>Nombre</th><th>Kg max.</th><th>Volumen max.</th><th>Costo por KM</th><th>Descripcion</th><th>Estado</th></tr></thead>
        <tbody>' . $truckRows . '</tbody>
      </table>
    </div>
    <h3 class="app-form-wide">Agregar tipo de camion</h3>
    <label>Nombre<input name="new_truck_type[name]"></label>
    <label>Kg maximos<input name="new_truck_type[max_weight_kg]" type="number" min="0" step="0.01"></label>
    <label>Volumen maximo m³<input name="new_truck_type[max_volume_m3]" type="number" min="0" step="0.001"></label>
    <label>Costo por km<input name="new_truck_type[cost_per_km]" type="number" min="0" step="0.01"></label>
    <label class="app-form-wide">Descripcion<input name="new_truck_type[description]"></label>
...
function save_admin_settings(): void
{
    ...
    foreach ($allowed as $name) {
        ...
        if (in_array($name, ['freight_base_fee', 'freight_per_km', 'freight_min_fee', 'freight_round_trip_factor', 'tax_rate'], true)) {
            $number = (float) $value;
        ...
        if (in_array($name, ['offer_popup_enabled', 'offer_confetti_enabled'], true)) {
            $value = isset($_POST[$name]) ? '1' : '0';
        }
        $stmt->execute([$name, $value]);
    }
    $heroImage = uploaded_image_file('hero_image_file', CONTENT_UPLOAD_DIR, CONTENT_UPLOAD_BASE);
    if ($heroImage !== '') {
        $stmt->execute(['hero_image', $heroImage]);
    }
    $postedTruckTypes = $_POST['truck_types'] ?? [];
    if (is_array($postedTruckTypes)) {
        $truckStmt = db()->prepare('UPDATE freight_truck_types SET name = ?, max_weight_kg = ?, max_volume_m3 = ?, cost_per_km = ?, description = ?, is_active = ? WHERE id = ?');
        foreach (freight_truck_types(false) as $truckType) {
            $truckId = (int) $truckType['id'];
            ...
            $truckStmt->execute([
                $name,
                max(0.0, (float) ($posted['max_weight_kg'] ?? 0)),
                max(0.0, (float) ($posted['max_volume_m3'] ?? 0)),
                $costPerKm,
                trim((string) ($posted['description'] ?? '')),
                $isActive,
                $truckId,
            ]);
        }
    }
    $newTruck = $_POST['new_truck_type'] ?? [];
    if (is_array($newTruck) && trim((string) ($newTruck['name'] ?? '')) !== '') {
        $name = trim((string) $newTruck['name']);
        db()->prepare(
            'INSERT INTO freight_truck_types (slug, name, max_weight_kg, max_volume_m3, cost_per_km, description, is_active, sort_order) VALUES (?, ?, ?, ?, ?, ?, 1, ?)'
        )->execute([
            slugify($name) . '-' . substr(bin2hex(random_bytes(3)), 0, 6),
            $name,
            max(0.0, (float) ($newTruck['max_weight_kg'] ?? 0)),
            max(0.0, (float) ($newTruck['max_volume_m3'] ?? 0)),
            max(0.0, (float) ($newTruck['cost_per_km'] ?? 0)),
            trim((string) ($newTruck['description'] ?? '')),
            count(freight_truck_types(false)) * 10 + 10,
        ]);
    }
...
function admin_users_page(): void
{
    ...
    foreach (db()->query('SELECT * FROM users ORDER BY created_at DESC')->fetchAll() as $user) {
        $documents = db()->prepare('SELECT id, document_type FROM user_documents WHERE user_id = ? ORDER BY document_type');
        $documents->execute([(int) $user['id']]);
        $documentLinks = [];
        foreach ($documents->fetchAll() as $document) {
            $documentLinks[] = '<a href="/admin/documentos/' . (int) $document['id'] . '" target="_blank">' . h(legal_document_types()[$document['document_type']] ?? $document['document_type']) . '</a>';
        }
        $rows .= '
<tr>
  ...
  <td>' . ($documentLinks ? implode('<br>', $documentLinks) : 'Sin documentos') . '</td>
...
<main class="app-admin-shell">
  ...
  <div class="app-admin-card">
    <table class="app-admin-table">
      <thead><tr><th>Usuario</th><th>Empresa</th><th>Telefono</th><th>Estado</th><th>Documentos</th><th>Alta</th></tr></thead>
      <tbody>' . ($rows ?: '<tr><td colspan="6">Todavia no hay usuarios.</td></tr>') . '</tbody>
    </table>
  </div>
</main>', 'admin');
}

function admin_logistics_page(): void
{
    $companyRows = '';
    foreach (db()->query('SELECT * FROM logistics_companies ORDER BY name')->fetchAll() as $company) {
        $portalUrl = absolute_url('/logistica/' . (string) $company['access_token']);
        $companyRows .= '<tr><td><input name="companies[' . (int) $company['id'] . '][name]" value="' . h($company['name']) . '" required></td><td><input name="companies[' . (int) $company['id'] . '][contact_name]" value="' . h($company['contact_name']) . '"></td><td><input name="companies[' . (int) $company['id'] . '][email]" type="email" value="' . h($company['email']) . '"></td><td><input name="companies[' . (int) $company['id'] . '][phone]" value="' . h($company['phone']) . '"></td><td><input name="companies[' . (int) $company['id'] . '][notes]" value="' . h($company['notes']) . '"></td><td><label class="app-check"><input name="companies[' . (int) $company['id'] . '][is_active]" type="checkbox" value="1" ' . ((int) $company['is_active'] === 1 ? 'checked' : '') . '> Activa</label></td><td><a href="' . h($portalUrl) . '" target="_blank">Abrir portal</a></td></tr>';
    }
    $shipmentRows = '';
    $shipments = db()->query(
        "SELECT shipments.*, logistics_companies.name AS company_name, freight_truck_types.name AS truck_name
        FROM shipments
        LEFT JOIN logistics_companies ON logistics_companies.id = shipments.assigned_company_id
        LEFT JOIN freight_truck_types ON freight_truck_types.id = shipments.truck_type_id
        ORDER BY shipments.created_at DESC"
    )->fetchAll();
    foreach ($shipments as $shipment) {
        $statusOptions = '';
        foreach (['route_pending', 'pending', 'offered', 'taken', 'assigned', 'completed', 'cancelled'] as $status) {
            $statusOptions .= '<option value="' . h($status) . '"' . ($status === (string) $shipment['status'] ? ' selected' : '') . '>' . h(shipment_status_label($status)) . '</option>';
        }
        $distance = (int) ($shipment['distance_meters'] ?? 0) > 0
            ? number_format(((int) $shipment['distance_meters']) / 1000, 2, '.', ',') . ' km'
            : 'Sin distancia';
        $duration = format_route_duration((int) ($shipment['duration_traffic_seconds'] ?? 0));
        $routeError = trim((string) ($shipment['route_error'] ?? ''));
        $routeInfo = '<strong>' . h(route_status_label((string) ($shipment['route_status'] ?? 'pending_recalculation'))) . '</strong><br>'
            . h($distance . ($duration !== '' ? ' - ' . $duration . ' con trafico' : ''))
            . ($routeError !== '' ? '<br><span>' . h($routeError) . '</span>' : '');
        $routeLink = trim((string) ($shipment['route_url'] ?? '')) !== ''
            ? '<a href="' . h($shipment['route_url']) . '" target="_blank" rel="noopener">Abrir ruta</a>'
            : 'Sin enlace';
        $shipmentRows .= '<tr><td><a href="/admin/pedidos/' . (int) $shipment['order_id'] . '">Pedido #' . (int) $shipment['order_id'] . '</a><span>' . h($shipment['created_at']) . '</span></td><td><select name="shipments[' . (int) $shipment['id'] . '][status]">' . $statusOptions . '</select></td><td>' . h($shipment['company_name'] ?: 'Sin asignar') . '</td><td>' . h($shipment['truck_name'] ?: 'Sin camion') . '</td><td>' . h(labor_number((float) $shipment['total_weight_kg'])) . ' kg<br>' . h(labor_number((float) $shipment['total_volume_m3'])) . ' m³<br>' . ((int) $shipment['has_non_stackable'] === 1 ? 'Incluye no apilables' : 'Apilable') . '</td><td>' . $routeInfo . '</td><td>' . $routeLink . '</td></tr>';
    }
    layout('Logistica', '<main class="app-admin-shell"><div class="app-admin-header"><div><h1>Logistica</h1></div></div>' . admin_nav('logistica') . '
    <form action="/admin/logistica" method="post" class="app-admin-form">
      <h2 class="app-form-wide">Empresas de logistica</h2>
      <div class="app-form-wide app-admin-card app-inline-table-card"><table class="app-admin-table"><thead><tr><th>Empresa</th><th>Contacto</th><th>Email</th><th>Telefono</th><th>Notas</th><th>Estado</th><th>Portal</th></tr></thead><tbody>' . ($companyRows ?: '<tr><td colspan="7">No hay empresas registradas.</td></tr>') . '</tbody></table></div>
      <h3 class="app-form-wide">Agregar empresa</h3>
      <label>Nombre<input name="new_company[name]"></label><label>Contacto<input name="new_company[contact_name]"></label><label>Email<input name="new_company[email]" type="email"></label><label>Telefono<input name="new_company[phone]"></label><label class="app-form-wide">Notas<input name="new_company[notes]"></label>
      <div class="app-form-actions"><button class="btn-contacto w-button" type="submit">Guardar empresas</button></div>
    </form>
    <form action="/admin/logistica/envios" method="post" class="app-admin-card"><h2>Envios</h2><table class="app-admin-table"><thead><tr><th>Envio</th><th>Estado</th><th>Empresa</th><th>Camion</th><th>Carga</th><th>Calculo de ruta</th><th>Ruta</th></tr></thead><tbody>' . ($shipmentRows ?: '<tr><td colspan="7">No hay envios generados.</td></tr>') . '</tbody></table><div class="app-form-actions"><button class="btn-contacto w-button" type="submit">Actualizar envios</button></div></form></main>', 'admin');
}

function save_logistics_companies(): void
{
    $companies = $_POST['companies'] ?? [];
    if (is_array($companies)) {
        $stmt = db()->prepare('UPDATE logistics_companies SET name = ?, contact_name = ?, email = ?, phone = ?, notes = ?, is_active = ? WHERE id = ?');
        foreach ($companies as $id => $company) {
            ...
            $stmt->execute([
                ...
                trim((string) ($company['contact_name'] ?? '')),
                trim((string) ($company['email'] ?? '')),
                trim((string) ($company['phone'] ?? '')),
                trim((string) ($company['notes'] ?? '')),
                isset($company['is_active']) ? 1 : 0,
                (int) $id,
            ]);
        }
    }
    $new = $_POST['new_company'] ?? [];
    if (is_array($new) && trim((string) ($new['name'] ?? '')) !== '') {
        db()->prepare('INSERT INTO logistics_companies (name, contact_name, email, phone, notes, is_active, access_token) VALUES (?, ?, ?, ?, ?, 1, ?)')->execute([
            trim((string) $new['name']),
            trim((string) ($new['contact_name'] ?? '')),
            trim((string) ($new['email'] ?? '')),
            trim((string) ($new['phone'] ?? '')),
            trim((string) ($new['notes'] ?? '')),
            bin2hex(random_bytes(24)),
        ]);
    }
    db()->exec("UPDATE shipments SET status = 'offered' WHERE status = 'pending' AND assigned_company_id IS NULL AND EXISTS (SELECT 1 FROM logistics_companies WHERE is_active = 1)");
    redirect_to('/admin/logistica');
}

function save_shipment_statuses(): void
{
    $allowed = ['route_pending', 'pending', 'offered', 'taken', 'assigned', 'completed', 'cancelled'];
    $shipments = $_POST['shipments'] ?? [];
    if (is_array($shipments)) {
        $stmt = db()->prepare(
            "UPDATE shipments SET status = ?, completed_at = CASE WHEN ? = 'completed' THEN COALESCE(completed_at, datetime('now')) ELSE completed_at END WHERE id = ?"
        );
        foreach ($shipments as $id => $values) {
            $status = is_array($values) ? (string) ($values['status'] ?? '') : '';
            if (in_array($status, $allowed, true)) {
                $stmt->execute([$status, $status, (int) $id]);
            }
        }
    }
    redirect_to('/admin/logistica');
...
function logistics_company_by_token(string $token): ?array
{
    $stmt = db()->prepare('SELECT * FROM logistics_companies WHERE access_token = ? AND is_active = 1');
    $stmt->execute([$token]);
    $company = $stmt->fetch();
...
function logistics_portal_page(string $token, string $message = ''): void
{
    $company = logistics_company_by_token($token);
    if (!$company) {
        not_found();
        return;
    }
    $stmt = db()->prepare(
        "SELECT shipments.*, orders.scheduled_at, orders.id AS order_number, freight_truck_types.name AS truck_name
        FROM shipments
        JOIN orders ON orders.id = shipments.order_id
        LEFT JOIN freight_truck_types ON freight_truck_types.id = shipments.truck_type_id
        WHERE (shipments.status = 'offered' AND shipments.assigned_company_id IS NULL)
           OR shipments.assigned_company_id = ?
        ORDER BY shipments.created_at DESC"
    );
    $stmt->execute([(int) $company['id']]);
    $cards = '';
    foreach ($stmt->fetchAll() as $shipment) {
        $assignedHere = (int) ($shipment['assigned_company_id'] ?? 0) === (int) $company['id'];
        $action = !$assignedHere && (string) $shipment['status'] === 'offered'
            ? '<form action="/logistica/' . h($token) . '/envios/' . (int) $shipment['id'] . '/aceptar" method="post"><button type="submit">Aceptar envio</button></form>'
            : '<strong>' . h(shipment_status_label((string) $shipment['status'])) . '</strong>';
        $distance = (int) ($shipment['distance_meters'] ?? 0) > 0
            ? number_format(((int) $shipment['distance_meters']) / 1000, 2, '.', ',') . ' km'
            : 'Pendiente';
        $duration = format_route_duration((int) ($shipment['duration_traffic_seconds'] ?? 0));
        $routeLink = trim((string) ($shipment['route_url'] ?? '')) !== ''
            ? '<a href="' . h($shipment['route_url']) . '" target="_blank" rel="noopener">Abrir recorrido en Google Maps</a>'
            : '';
        $cards .= '<article class="app-shipment-card"><h2>Pedido #' . (int) $shipment['order_number'] . '</h2><p>Fecha y hora: ' . h((string) $shipment['scheduled_at']) . '</p><p>Camion: ' . h($shipment['truck_name'] ?: 'Por definir') . '</p><p>Ruta: ' . h($distance . ($duration !== '' ? ' - ' . $duration . ' con trafico' : '')) . '</p><p>Carga: ' . h(labor_number((float) $shipment['total_weight_kg'])) . ' kg · ' . h(labor_number((float) $shipment['total_volume_m3'])) . ' m³</p><p>' . ((int) $shipment['has_non_stackable'] === 1 ? 'Incluye productos no apilables.' : 'Carga apilable.') . '</p>' . $routeLink . $action . '</article>';
    }
    layout('Portal de logistica', '<main class="app-admin-shell"><div class="app-admin-header"><div><p class="app-kicker">Logistica</p><h1>' . h($company['name']) . '</h1></div></div>' . ($message !== '' ? '<div class="app-contact-feedback">' . h($message) . '</div>' : '') . '<section class="app-shipment-grid">' . ($cards ?: '<p>No hay ofertas disponibles.</p>') . '</section></main>');
}

function accept_logistics_shipment(string $token, int $shipmentId): void
{
    $company = logistics_company_by_token($token);
    if (!$company) {
        not_found();
    ...
    $stmt = db()->prepare(
        "UPDATE shipments SET assigned_company_id = ?, status = 'taken', accepted_at = datetime('now')
        WHERE id = ? AND assigned_company_id IS NULL AND status = 'offered'"
    );
    $stmt->execute([(int) $company['id'], $shipmentId]);
    logistics_portal_page($token, $stmt->rowCount() === 1 ? 'Envio asignado correctamente.' : 'El envio ya fue tomado por otra empresa.');
}

function admin_blog_page(?int $editId = null): void
{
    $post = [
        'id' => 0, 'slug' => '', 'title' => '', 'summary' => '', 'image_url' => '',
        'content' => '', 'meta_description' => '', 'published_at' => date('Y-m-d H:i:s'), 'is_published' => 1,
    ];
    if ($editId) {
    ...
    $rows = '';
    foreach (blog_posts(false) as $row) {
        $rows .= '<tr><td><strong>' . h($row['title']) . '</strong><span>/' . h($row['slug']) . '</span></td><td>' . h(substr((string) $row['published_at'], 0, 10)) . '</td><td>' . ((int) $row['is_published'] === 1 ? 'Publicado' : 'Borrador') . '</td><td class="app-admin-actions"><a href="/admin/blog/' . (int) $row['id'] . '/editar">Editar</a><a href="/blog/' . h($row['slug']) . '" target="_blank">Ver</a></td></tr>';
    }
    layout('Blog admin', '<main class="app-admin-shell"><div class="app-admin-header"><div><h1>Blog</h1></div></div>' . admin_nav('blog') . '
    <form action="/admin/blog/guardar" method="post" enctype="multipart/form-data" class="app-admin-form">
      <input type="hidden" name="id" value="' . (int) $post['id'] . '">
      <label>Titulo<input name="title" value="' . h($post['title']) . '" required></label>
      <label>Slug<input name="slug" value="' . h($post['slug']) . '"></label>
      <label class="app-form-wide">Resumen<textarea name="summary" rows="3" required>' . h($post['summary']) . '</textarea></label>
      <label class="app-form-wide">Imagen actual o URL<input name="image_url" value="' . h($post['image_url']) . '"></label>
      <label class="app-form-wide">Subir imagen<input name="blog_image" type="file" accept="image/jpeg,image/png,image/webp,image/avif"></label>
      <label class="app-form-wide">Contenido<textarea name="content" rows="12" required>' . h($post['content']) . '</textarea></label>
      <label class="app-form-wide">Meta description<input name="meta_description" maxlength="180" value="' . h($post['meta_description']) . '"></label>
      <label>Fecha<input name="published_at" type="datetime-local" value="' . h(str_replace(' ', 'T', substr((string) $post['published_at'], 0, 16))) . '"></label>
      <label class="app-check"><input name="is_published" type="checkbox" value="1" ' . ((int) $post['is_published'] === 1 ? 'checked' : '') . '> Publicado</label>
      <div class="app-form-actions"><button class="btn-contacto w-button" type="submit">Guardar articulo</button></div>
    </form>
    <section class="app-admin-card"><table class="app-admin-table"><thead><tr><th>Articulo</th><th>Fecha</th><th>Estado</th><th></th></tr></thead><tbody>' . ($rows ?: '<tr><td colspan="4">No hay articulos.</td></tr>') . '</tbody></table></section></main>', 'admin');
}

function save_blog_post(): void
{
    $id = (int) ($_POST['id'] ?? 0);
    ...
    $image = uploaded_image_file('blog_image', CONTENT_UPLOAD_DIR, CONTENT_UPLOAD_BASE);
    if ($image === '') {
        $image = trim((string) ($_POST['image_url'] ?? ''));
    }
    $values = [
        trim((string) ($_POST['slug'] ?? '')) ?: slugify($title),
        $title,
        trim((string) ($_POST['summary'] ?? '')),
        $image,
        trim((string) ($_POST['content'] ?? '')),
        trim((string) ($_POST['meta_description'] ?? '')),
        normalize_datetime_value((string) ($_POST['published_at'] ?? '')) ?: date('Y-m-d H:i:s'),
        isset($_POST['is_published']) ? 1 : 0,
    ];
    if ($id > 0) {
        db()->prepare('UPDATE blog_posts SET slug = ?, title = ?, summary = ?, image_url = ?, content = ?, meta_description = ?, published_at = ?, is_published = ? WHERE id = ?')->execute([...$values, $id]);
    } else {
        db()->prepare('INSERT INTO blog_posts (slug, title, summary, image_url, content, meta_description, published_at, is_published) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')->execute($values);
        $id = (int) db()->lastInsertId();
    }
    redirect_to('/admin/blog/' . $id . '/editar');
}
...
<main class="app-admin-shell">
  ...
  <form action="' . h($action) . '" method="post" class="app-admin-form" enctype="multipart/form-data">
    ...
    <h2 class="app-form-wide">Datos logisticos</h2>
    <label>Peso por unidad (kg)<input name="weight_kg" type="number" min="0" step="0.01" value="' . h($product['weight_kg'] ?? 0) . '"></label>
    <label>Volumen por unidad (m³)<input name="volume_m3" type="number" min="0" step="0.001" value="' . h($product['volume_m3'] ?? 0) . '"></label>
    <label class="app-check"><input type="checkbox" name="is_stackable" value="1" ' . ((int) ($product['is_stackable'] ?? 0) === 1 ? 'checked' : '') . '> Producto apilable</label>
    <label class="app-check"><input type="checkbox" name="requires_area_m2" value="1" ' . ((int) ($product['requires_area_m2'] ?? 0) === 1 ? 'checked' : '') . '> Requiere m² de losa antes del alquiler</label>
...
function save_product(?int $productId = null): array
{
    ...
    $values = [
        ...
        ':weight_kg' => max(0, (float) ($_POST['weight_kg'] ?? 0)),
        ':volume_m3' => max(0, (float) ($_POST['volume_m3'] ?? 0)),
        ':is_stackable' => isset($_POST['is_stackable']) ? 1 : 0,
        ':requires_area_m2' => isset($_POST['requires_area_m2']) ? 1 : 0,
    ];
    ...
    if ($productId) {
        ...
        db()->prepare(
            "UPDATE products SET
                ...
                labor_min_units = :labor_min_units, weight_kg = :weight_kg,
                volume_m3 = :volume_m3, is_stackable = :is_stackable,
                requires_area_m2 = :requires_area_m2
            WHERE id = :id"
        )->execute($values);
    } else {
        db()->prepare(
            "INSERT INTO products (
                ...
                , weight_kg, volume_m3, is_stackable, requires_area_m2
            ) VALUES (
                :slug, :name, :code, :brand, :category, :specialization, :short_description,
                :description, :status, :price_sale_used, :price_sale_new, :rental_daily,
                :rental_weekly, :rental_monthly, :images, :specs, :is_featured, :is_new,
...
function api_reservation(): void
{
    ...
    if ((int) ($product['requires_area_m2'] ?? 0) === 1 && (float) ($payload['area_m2'] ?? 0) <= 0) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Indica para cuantos m² de losa necesitas los casetones.']);
        return;
    }
...
function normalize_checkout_item(array $item, ?array $product = null): array
{
    ...
    if (!$product) {
        throw new InvalidArgumentException('El producto del carrito no es valido.');
    }
    $unitPrice = 0.0;
    if ($mode === 'rental') {
        $unitPrice = match ((string) ($item['rental_plan'] ?? '')) {
            'semanal' => (float) ($product['rental_weekly'] ?? 0),
            'mensual' => (float) ($product['rental_monthly'] ?? 0),
            default => (float) ($product['rental_daily'] ?? 0),
        };
    } elseif ($mode === 'purchase') {
        $priceLabel = strtolower((string) ($item['price_label'] ?? ''));
        $unitPrice = str_contains($priceLabel, 'nuevo')
            ? (float) ($product['price_sale_new'] ?? 0)
            : (float) ($product['price_sale_used'] ?? 0);
    } else {
        throw new InvalidArgumentException('La modalidad del producto no es valida.');
    }
    ...
    $areaM2 = max(0.0, (float) ($item['area_m2'] ?? 0));
    if ((int) ($product['requires_area_m2'] ?? 0) === 1 && $areaM2 <= 0) {
        throw new InvalidArgumentException('Indica para cuantos m² de losa necesitas los casetones Atex.');
    }
    $details = $areaM2 > 0 ? ['kind' => 'product', 'area_m2' => $areaM2] : [];
...
function api_checkout(): void
{
    ...
    $fulfillmentType = (string) ($customer['fulfillment_type'] ?? '');
    if (!in_array($fulfillmentType, ['pickup', 'delivery'], true)) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Selecciona retiro por cuenta propia o entrega con logistica.']);
        return;
    }
    $scheduledAt = normalize_datetime_value((string) ($customer['scheduled_at'] ?? ''));
    if ($scheduledAt === '') {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Selecciona la fecha y hora de entrega o retiro.']);
        return;
    }
    $requiredFields = [];
    if ($fulfillmentType === 'delivery') {
        $requiredFields = [
            'delivery_type' => 'Selecciona el destino de entrega.',
            'city' => 'Selecciona la ciudad de entrega.',
            'address' => 'Selecciona la direccion de entrega.',
            'delivery_lat' => 'Selecciona un pin valido en el mapa para la entrega.',
            'delivery_lng' => 'Selecciona un pin valido en el mapa para la entrega.',
    ...
    if ($fulfillmentType === 'delivery' && !in_array((string) ($customer['delivery_type'] ?? ''), ['empresa', 'obra'], true)) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Selecciona un tipo de entrega valido.']);
        return;
    }
    if ($fulfillmentType === 'delivery' && trim((string) ($customer['city'] ?? '')) === 'Santo Domingo' && trim((string) ($customer['delivery_zone'] ?? '')) === '') {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
    ...
    $deliveryTypeLabel = $fulfillmentType === 'pickup'
        ? 'Retiro por cuenta propia'
        : ($deliveryType === 'empresa' ? 'Entrega en Empresa' : ($deliveryType === 'obra' ? 'Entrega en Obra' : $deliveryType));
    $freightSettings = freight_settings();
    $originLatLng = valid_lat_lng($freightSettings['origin_lat'], $freightSettings['origin_lng']);
    $destinationLatLng = valid_lat_lng($customer['delivery_lat'] ?? null, $customer['delivery_lng'] ?? null);
    if ($fulfillmentType === 'delivery' && !$destinationLatLng) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Selecciona un pin valido en el mapa para la entrega.']);
        return;
    }
    if ($fulfillmentType === 'delivery' && !$originLatLng) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Las coordenadas de la base de Inexo todavia no estan configuradas. Contacta al equipo comercial.']);
        return;
    }
    $truckTypeId = (int) ($customer['freight_truck_type_id'] ?? 0);
    $truckType = $fulfillmentType === 'delivery' ? freight_truck_type_by_id($truckTypeId, true) : null;
    if ($fulfillmentType === 'delivery' && !$truckType) {
        http_response_code(422);
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(['ok' => false, 'message' => 'Selecciona un tipo de camion activo para el flete.']);
        return;
    }
    $route = $fulfillmentType === 'delivery'
        ? compute_google_route($originLatLng, $destinationLatLng)
        : [
    ...
    $browserEstimateKm = max(0.0, (float) ($customer['delivery_distance_km'] ?? 0));
    $deliveryDistanceKm = $route['status'] === 'calculated'
        ? round(((int) $route['distance_meters']) / 1000, 3)
        : $browserEstimateKm;
    ...
    $freightAmount = $fulfillmentType === 'delivery' && $truckType ? calculate_freight_amount($deliveryDistanceKm, $truckType) : 0.0;
    $deliverySnapshot = [
        'type' => $deliveryType,
        ...
        'route_error' => (string) $route['error'],
        'route_calculated_at' => (string) $route['calculated_at'],
        'distance_source' => $route['status'] === 'calculated' ? 'google_routes_api' : ($browserEstimateKm > 0 ? 'browser_estimate' : 'pending'),
        'fulfillment_type' => $fulfillmentType,
        'scheduled_at' => $scheduledAt,
        'freight_truck_type_id' => (int) ($truckType['id'] ?? 0),
    ...
    $deliverySnapshot['route_url'] = $fulfillmentType === 'delivery' ? google_maps_route_url($deliverySnapshot) : '';
    $normalizedItems = [];
    $subtotalAmount = 0.0;
    $totalWeight = 0.0;
    $totalVolume = 0.0;
    ...
    foreach ($items as $item) {
        ...
        $quantity = max(1, (int) ($item['qty'] ?? 1));
        $totalWeight += max(0.0, (float) ($product['weight_kg'] ?? 0)) * $quantity;
        $totalVolume += max(0.0, (float) ($product['volume_m3'] ?? 0)) * $quantity;
        if ((int) ($product['is_stackable'] ?? 0) !== 1) {
            $hasNonStackable = 1;
        }
    }
    if ($normalizedItems === []) {
    ...
    if ($fulfillmentType === 'delivery' && $truckType) {
        $maxWeight = max(0.0, (float) ($truckType['max_weight_kg'] ?? 0));
        $maxVolume = max(0.0, (float) ($truckType['max_volume_m3'] ?? 0));
        if ($maxWeight > 0 && $totalWeight > $maxWeight) {
            http_response_code(422);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['ok' => false, 'message' => 'El peso del pedido supera la capacidad del camion seleccionado.']);
            return;
        }
        if ($maxVolume > 0 && $totalVolume > $maxVolume) {
            http_response_code(422);
            header('Content-Type: application/json; charset=utf-8');
            echo json_encode(['ok' => false, 'message' => 'El volumen del pedido supera la capacidad del camion seleccionado.']);
            return;
        }
    }
    $tax = tax_settings();
    $taxableAmount = round($subtotalAmount + $freightAmount, 2);
    $taxAmount = round($taxableAmount * ($tax['rate'] / 100), 2);
    $totalAmount = round($taxableAmount + $taxAmount, 2);
    $orderStatus = $fulfillmentType === 'delivery' && $route['status'] !== 'calculated'
        ? 'ruta_pendiente_recalcular'
        : 'validado';
    ...
    try {
        ...
        $pdo->prepare(
            'INSERT INTO orders (
                ...
                subtotal_amount, freight_amount, tax_name, tax_rate, tax_amount,
                total_amount, fulfillment_type, scheduled_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
        )->execute([
            $userId,
            $orderStatus,
        ...
        if ($fulfillmentType === 'delivery') {
            $activeLogistics = (int) $pdo->query('SELECT COUNT(*) FROM logistics_companies WHERE is_active = 1')->fetchColumn();
            $shipmentStatus = $route['status'] === 'calculated'
                ? ($activeLogistics > 0 ? 'offered' : 'pending')
                : 'route_pending';
            $pdo->prepare(
                'INSERT INTO shipments (
                    order_id, status, truck_type_id, origin_address, origin_lat, origin_lng,
                    destination_address, destination_lat, destination_lng, route_url,
                    route_status, distance_meters, duration_seconds, duration_traffic_seconds,
                    encoded_polyline, route_error, route_calculated_at,
                    total_weight_kg, total_volume_m3, has_non_stackable
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
            )->execute([
                $orderId,
                $shipmentStatus,
                $truckTypeId,
                $deliverySnapshot['origin_address'],
                $deliverySnapshot['origin_lat'],
                $deliverySnapshot['origin_lng'],
                $deliverySnapshot['address'],
                $deliverySnapshot['lat'],
                $deliverySnapshot['lng'],
                $deliverySnapshot['route_url'],
                $deliverySnapshot['route_status'],
                $deliverySnapshot['distance_meters'],
                $deliverySnapshot['duration_seconds'],
                $deliverySnapshot['duration_traffic_seconds'],
                $deliverySnapshot['route_polyline'],
                $deliverySnapshot['route_error'],
                $deliverySnapshot['route_calculated_at'] ?: null,
                round($totalWeight, 2),
                round($totalVolume, 3),
                $hasNonStackable,
            ]);
    ...
    $proformaSent = false;
    $proformaError = '';
    if ($route['status'] === 'calculated' || $fulfillmentType === 'pickup') {
        try {
        ...
        $proformaError = 'ruta_pendiente_recalcular';
    }
    ...
    echo json_encode([
        'ok' => true,
        'message' => $route['status'] !== 'calculated' && $fulfillmentType === 'delivery'
            ? 'Pedido registrado. El equipo de Inexo confirmara la ruta y el flete antes de enviar la proforma.'
            : ($proformaSent
...
function dispatch(): void
{
    ...
    if ($method === 'POST') {
        ...
        if (preg_match('#^/admin/pedidos/(\d+)/recalcular-ruta$#', $path, $match)) {
            recalculate_order_route((int) $match[1]);
            return;
        }
        ...
        if ($path === '/admin/logistica') {
            save_logistics_companies();
            return;
        }
        if ($path === '/admin/logistica/envios') {
            save_shipment_statuses();
            return;
        }
        if ($path === '/admin/blog/guardar') {
            save_blog_post();
            return;
        }
        if (preg_match('#^/logistica/([a-f0-9]{48})/envios/(\d+)/aceptar$#', $path, $match)) {
            accept_logistics_shipment($match[1], (int) $match[2]);
            return;
        }
    }
    ...
    if (preg_match('#^/blog/([^/]+)$#', $path, $match)) {
        blog_article_page(urldecode($match[1]));
        return;
    }
    ...
    if (preg_match('#^/cuenta/documentos/(\d+)$#', $path, $match)) {
        output_user_document((int) $match[1]);
        return;
    }
    if ($path === '/salir') {
        session_destroy();
    ...
    if (preg_match('#^/admin/blog/(\d+)/editar$#', $path, $match)) {
        admin_blog_page((int) $match[1]);
        return;
    }
    if (preg_match('#^/admin/documentos/(\d+)$#', $path, $match)) {
        output_user_document((int) $match[1], true);
        return;
    }
    ...
    if (preg_match('#^/logistica/([a-f0-9]{48})$#', $path, $match)) {
        logistics_portal_page($match[1]);
        return;
    }
...
if (getenv('INEXO_SKIP_DISPATCH') !== '1') {
    dispatch();
}
